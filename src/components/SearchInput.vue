...
import { reactive } from 'vue';
    ...
    const handleSearch = () => {
        ...
        searchTerm.timeout = setTimeout(async() =>{
            ...
            const res = await fetch(
                `http://api.weatherapi.com/v1/search.json?key=a58567e826db4c18bf072816252203&q=${searchTerm.query}`
            )
    ...
    const getWeather = async(id) => {
        const res = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=a58567e826db4c18bf072816252203&q=id:${id}&days=3&aqi=no&alerts=no`)
